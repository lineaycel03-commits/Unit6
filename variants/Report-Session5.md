...
We used FreeBayes to identify single nucleotide polymorphisms (SNPs) and small insertions/deletions (indels), setting the ploidy to two. Variant calling generated an initial VCF file containing 31,521 raw variants. To remove low-confidence and potentially artifactual calls, we applied a quality filter (QUAL < 1) using bcftools filter, which reduced the dataset to 30,490 high-confidence variants, including 25,441 SNPs and 2,395 indels. We then visualized and validated these variants using the Integrative Genomics Viewer (IGV). 

We performed a detailed inspection of selected genomic regions in IGV; for example, at position chr10:9,058,200, we identified a homozygous insertion, where the reference sequence CAAAGGC was replaced by CAAAAGGC. In addition, we verified that SNPs located at chr10:10,000,166 fall within an intergenic region, as no overlapping gene models were observed in the annotation track.
